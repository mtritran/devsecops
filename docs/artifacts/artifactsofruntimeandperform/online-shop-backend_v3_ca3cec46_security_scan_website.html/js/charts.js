function index_to_shortname( index ){
    return ["common_files","x_frame_options","insecure_cors_policy"][index];
}

function index_to_severity( index ){
    return {"common_files":"low","x_frame_options":"low","insecure_cors_policy":"low"}[index_to_shortname(index)];
}
...
function renderCharts() {
    ...
    c3.generate({
        ...
        axis: {
            x: {
                type: 'category',
                categories: ["Common sensitive file","Missing 'X-Frame-Options' header","Insecure 'Access-Control-Allow-Origin' header"],
                tick: {
                    rotate: 15
                }
            },
